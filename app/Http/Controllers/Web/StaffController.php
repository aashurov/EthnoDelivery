...
class StaffController extends Controller
{
    ...
    public function listofsendedparcel($id)
    {
        ...
        return view('staff.parcel.listofsendedparcel', ['listofparcels' => $myTask, 'title' => $title, 'userBranch_id' => $userBranch_id], compact(['user']));
    }
    public function listofwaitingparcel($id)
    {
        ...
        $lw = 1;
        return view('staff.parcel.listofwaitingparcel', ['listofparcels' => $myTask, 'title' => $title, 'userBranch_id' => $recipientBranch_id, 'lw' => $lw], compact(['user']));
    }
    public function addparcel()
    {
        ...
        return view('staff.parcel.addparcel', [
            // 'recipientUser_id' => $recipientUser_id,
    ...
    public function savespecialparcel(Request $request)
    {
        ...
        return redirect()->route('staff.parcel.listofsendedparcel',  compact(['user', 'id']))->with('status', 'Новая посылка успешно добавлена');
    }
    ...
    public function delete($id, $refNumber)
    {
        ...
        $title = 'Все отправленные посылки';
        $id = 0;
        return redirect()->route('staff.parcel.listofsendedparcel', compact(['user', 'title', 'id']))->with('status', 'Посылка успешно удалена');
    }
    public function viewparcel($id)
    {
        ...
        return view('staff.parcel.viewparcel', ['parcel' => $myTask, 'historys' => $myTask1], compact(['user']));
    }
    public function editparcel($id)
    {
        ...
        return view('staff.parcel.editparcel', [
            'branches'   => $branches,
            'parcelPlanName' => $parcelPlanName,
            'users' => $users,
            'parcel' => $myTask1
    ...
    public function updateparcel(Request $request, $id)
    {
        ...
        $title = 'Все отправленные посылки';
        $id = 0;
        return redirect()->route('staff.parcel.listofsendedparcel', compact(['user', 'id']))->with('status', 'Посылка успешно обновлена');
    }
